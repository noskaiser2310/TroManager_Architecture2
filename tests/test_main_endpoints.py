...
def client(mock_dependencies):
    ...
    # Lưu env state trước khi import main
    import os
...
def test_list_approvals_returns_empty(client, mock_dependencies):
    ...
    response = client.get("/admin/approvals?status=pending")
    assert response.status_code == 200
    data = response.json()
    assert data["count"] == 0
...
def test_list_approvals_caps_limit(client, mock_dependencies):
    ...
    mock_dependencies["conn"].fetch = AsyncMock(return_value=[])
    response = client.get("/admin/approvals?limit=99999")
    assert response.status_code == 200
...
def test_approve_already_approved(client, mock_dependencies):
    ...
    response = client.post("/admin/approvals/1/approve", json={})
    assert response.status_code == 400
...
def test_rate_limit_stats(client_with_rate_limit):
    ...
    response = client_with_rate_limit.get("/admin/rate-limit/stats/tenant:1")
    assert response.status_code == 200
    data = response.json()
...
def test_rate_limit_reset(client_with_rate_limit):
    ...
    response = client_with_rate_limit.post("/admin/rate-limit/reset/tenant:1")
    assert response.status_code == 200
