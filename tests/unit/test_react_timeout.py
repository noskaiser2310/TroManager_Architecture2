...
def _make_agent(config_overrides=None) -> tuple:
    ...
    agent = ReActAgent(
        config=config,
        llm_client=llm,
        system_prompt="{tenant_name} {tone} {lease_end} {payment_delay_days} {memories} {behavior_summary} {query} {history_context} {tool_descriptions} {current_date}",
...
def test_default_tool_timeout_is_30_seconds():
    """Default tool timeout là 30s."""
...
async def test_llm_timeout_does_not_block_forever():
    ...
    # Phải < 10s (3 retries + backoff ~5.4s)
    assert elapsed < 10.0, f"LLM timeout không work, elapsed={elapsed:.2f}s"
