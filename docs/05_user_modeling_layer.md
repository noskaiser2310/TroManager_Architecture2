...
- **Cập nhật**: Batch job cuối ngày (Persona Optimizer)
- **Đọc bởi**: System 2 (top-k similarity search)
- **Ví dụ memories**:
...
Cuối mỗi ngày, một batch job chạy:
```python
def persona_optimizer_daily():
    """Tổng hợp behavior logs thành semantic memories."""
    for tenant in get_all_active_tenants():
        recent_logs = get_recent_logs(tenant.id, days=30)
        summary = gemini_pro.generate(
            prompt=f"Phân tích 30 ngày hành vi của khách thuê và đúc kết thành 3-5 insights ngắn gọn:\n{recent_logs}",
            response_schema={"insights": list[str]}
        )
        for insight in summary["insights"]:
            embedding = get_embedding(insight)
            db.execute("""
                INSERT INTO user_embeddings (tenant_id, memory_text, embedding)
                VALUES (%s, %s, %s)
            """, tenant.id, insight, embedding)
```
...
### 5.1. Embedding Model
- **Model**: `nomic-embed-text` (qua Ollama local hoặc API)
- **Dimension**: 768
