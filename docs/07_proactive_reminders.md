...
## 2. Các Loại Sự Kiện Nền

| Event | Trigger | Priority | Tool cần |
|-------|---------|----------|----------|
| `invoice_overdue` | Hóa đơn quá hạn 1, 3, 7 ngày | High | automation |
| `payment_due_soon` | Hóa đơn sắp đến hạn (3 ngày trước) | Normal | automation |
| `contract_expiring` | Hợp đồng hết hạn trong 30 ngày | High | knowledge, automation |
| `lease_anniversary` | Kỷ niệm ngày thuê | Low | automation |
| `maintenance_reminder` | Đến lịch bảo trì định kỳ | Normal | automation |
| `room_transfer_offer` | Có phòng phù hợp hơn cho tenant | Low | decision, automation |
| `birthday_greeting` | Sinh nhật tenant | Low | automation |
| `welcome_message` | Tenant mới move-in | Normal | knowledge, automation |
...
class EventDispatcher:
    ...
    async def dispatch(self, event_type: str, tenant_id: int, data: dict, instruction: str):
        ...
        # 2. Select tools based on event type
    ...
    def _event_to_intent(self, event_type: str) -> str:
        mapping = {
            "invoice_overdue": "payment_reminder",
            "payment_due_soon": "payment_reminder",
            "contract_expiring": "contract_renewal",
            "maintenance_reminder": "maintenance_status",
            "room_transfer_offer": "room_transfer",
            "birthday_greeting": "general_chat",
            "welcome_message": "general_chat"
        }
        return mapping.get(event_type, "general_chat")
```
...
```python
def select_channel(tenant_id: int, event_type: str) -> str:
    profile = db.get_profile(tenant_id)
    
    # Theo preference
    if profile.communication_preference == "zalo":
        return "zalo"
    elif profile.communication_preference == "sms":
        return "sms"
    
    # Theo urgency
    if event_type in ["invoice_overdue", "contract_expiring"]:
        # Quan trọng → cả Zalo + SMS
        return "multi"
    else:
        return "zalo"  # Default
```
...
- `../src/cron/detectors/` - Các event detector
