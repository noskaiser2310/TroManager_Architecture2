...
- **Latency thấp** (< 2s)
- **Chi phí thấp** (`gemma-4-31b-it` qua proxy)
...
def get_embedding(text: str) -> list[float]:
    return embedding_model.encode(text).tolist()  # 3072 dim
```
...
### 5.2. Confidence Escalation (duy nhất)
- Nếu confidence < 0.7 → fallback System 2
- Nếu confidence > 0.85 → save to cache (async, không block)

**Lưu ý**: Hallucination Check và Safety Keywords hiện chưa được implement (khác với thiết kế ban đầu).
